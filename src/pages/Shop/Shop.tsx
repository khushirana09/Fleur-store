...
// Page title mapping per navbar link
...
const SORT_TITLES: Record<string, string> = {
    newest: 'New In',
    'price-asc': 'Sale — Best Prices',
    'price-desc': 'Premium Collection',
    rating: 'Top Rated',
...
export function Shop() {
    ...
    // ── KEY FIX: searchParams is now in dependency array ──
    // This re-runs every time the URL changes (clicking navbar links)
    useEffect(() => {
        const cat = searchParams.get('category')
        const sortP = searchParams.get('sort') as SortOption | null
        const search = searchParams.get('q')

        // Reset everything first so previous state doesn't bleed through
        if (cat) {
            dispatch(setActiveCategory(cat))
            dispatch(setSort('featured'))
        } else if (sortP) {
            dispatch(setActiveCategory('all'))
            dispatch(setSort(sortP))
        } else if (!search) {
            // Plain /shop — show all
            dispatch(setActiveCategory('all'))
            dispatch(setSort('featured'))
        ...
        dispatch(setPage(1))
    }, [searchParams, dispatch]) // ← searchParams in deps = re-runs on URL change
    ...
    // Smart page title based on what's active
    const pageTitle = searchQuery
        ? `Results for "${searchQuery}"`
        : activeCategory !== 'all'
            ? PAGE_TITLES[activeCategory] ?? activeCategory
            : SORT_TITLES[sort] ?? 'All Products'

    // Breadcrumb label
    const breadcrumbLabel = activeCategory !== 'all'
        ? PAGE_TITLES[activeCategory] ?? activeCategory
        : pageTitle
                ...
                <div className="container-app py-8">
                    <Breadcrumb
                        items={[
                            ...
                            ...(activeCategory !== 'all' || sort !== 'featured'
                ...
                <div className="flex items-center justify-between gap-3 mb-6">
                    ...
                    {searchQuery && (
                        <div className="flex items-center gap-2">
                            <span className="text-sm text-mauve-400">Search:</span>
                            <span className="flex items-center gap-1.5 px-3 py-1 bg-rose-50
                                border border-rose-100 rounded-full text-sm text-mauve-700">
                                {searchQuery}
                                <button
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                        ...
                        <select
                            ...
                            className="border border-rose-100 rounded-xl px-3 py-2 text-sm
                                text-mauve-700 focus:outline-none focus:border-rose-300
                ...
                <div className="flex gap-2 overflow-x-auto pb-3 mb-6 scrollbar-hide">
                    {[
                        ...
                        <button
                            ...
                            className={cn(
                                'shrink-0 px-4 py-1.5 rounded-full text-[12px] tracking-wide',
                                'border transition-all duration-200 font-medium',
                                activeCategory === cat.value
                                    ? 'bg-rose-500 border-rose-500 text-white'
                                    : 'border-rose-100 text-mauve-500 hover:border-rose-300 hover:text-rose-500'
                            )}
                        >
                            {cat.label}
                        </button>
                            ...
                            <>
                                ...
                                <motion.div
                                    ...
                                    className="fixed left-0 top-0 bottom-0 z-50 bg-white
                                        border-r border-rose-100 overflow-y-auto p-5
                                        lg:hidden w-[85vw] max-w-[300px]"
                                >
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                p === page
                                                    ...
                                                    : 'text-mauve-500 hover:bg-rose-50 hover:text-rose-500'
                                            )}
                                        >
                                            {p}
                                        </button>
